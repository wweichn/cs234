...
def update_mdp_model_with_history(counts, rewards, history):
  ...
  for i in range(len(history)):
    (state, action, reward, next_state, done) = history[i]
    counts[state][action][next_state] += 1
    rewards[state][action][next_state] = (rewards[state][action][next_state] + reward) / counts[state][action][next_state]

  ############################

  return counts, rewards

def render(env,policy,e):
...
def learn_with_mdp_model(env, num_episodes=5000, gamma = 0.95, e = 0.8, decay_rate = 0.99):
  ...
  for k in range(num_episodes):
      ...
      P = counts_and_rewards_to_P(counts, rewards)    #P[state][action] is a list of (prob, next_state, reward, done) tuples.
