...
class Linear(DQN):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope,reuse = reuse):
            flatten_state = tf.contrib.layers.flatten(state,scope)
            out = tf.contrib.layers.fully_connected(flatten_state, num_actions, activation_fn=None)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        pass
        train_vars1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope = q_scope)
        train_vars2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope = target_q_scope)
        op = [tf.assign(train_vars2[i],train_vars1[i]) for i in range(len(train_vars1))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        Q = tf.reduce_sum(tf.multiply(action,q),axis = 1)       #alone an axis
        self.loss = tf.reduce_mean(tf.square(Q - Q_samp))
        pass
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        grads,v = list(zip(*optimizer.compute_gradients(self.loss,variables)))
